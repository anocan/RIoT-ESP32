...
void compareAndUpdateRiotCard(const char* riotCardID, const char* comparisonValue, int* count) {
    FirebaseJson jsonRiotCard;
    char documentPath[64];
    sprintf(documentPath, "riotCards/%s", riotCardID);

    firestoreGetJson(&jsonRiotCard, documentPath);
    String userType = getDataFromJsonObject(&jsonRiotCard,
...
void updateRiotCardStatus() {
    ...
    while(
    riotCardsList.get(nextPageToken,
    "nextPageToken") || 
    firstPage
    ) {
        if (!firstPage) {
